module Viwrap.Pty.Handler ( runLoggerIO
                          , runPtyActIO
                          , runLoggerUnit
                          , ViwrapEffIO
                          , ViwrapEnv
...
import Control.Monad.Freer ( interpret , sendM, Member, Eff, LastMember, Members )
...
import System.Posix.Terminal (TerminalAttributes, TerminalState, TerminalMode(..))
...
runLoggerUnit = interpret
              $ \case
                   LogM _ _ -> return ()

runLoggerIO :: forall a effs. (ViwrapEnv effs, LastMember IO effs) => Eff (Logger ': effs) a -> Eff effs a
runLoggerIO = interpret
            $ \case
                LogM x (y:ys) -> do
                     Env{logFile} <- ask
...
runPtyActIO :: forall a effs. (LastMember IO effs, ViwrapEnv effs, Member Logger effs)
            => Eff (PtyAct ': effs) a -> Eff effs a
runPtyActIO = interpret
            $ \case
                 OpenPty -> openPtyIO
                 FdToHandle fd -> logM "FdToHandle" [printf "convert fd %d to handle" (toInteger fd)] >> sendM (IO.fdToHandle fd)
...
openPtyIO :: forall effs. (LastMember IO effs, Member Logger effs) => Eff effs (FdMaster, FdSlave)
openPtyIO = do
              let logOpenPty = logM "OpenPty" 
              logOpenPty ["opening pty..."]
              (masterFd, slaveFd) <- sendM Terminal.openPseudoTerminal

              logOpenPty [printf "Master: FD=%d, Slave: FD=%d"
                                 (toInteger masterFd)
                                 (toInteger slaveFd)]
...
hReadIO :: forall effs. (ViwrapEnv effs, LastMember IO effs, Member Logger effs)
        => Handle -> Eff effs (Maybe ByteString)
hReadIO handle = do
...
hWriteIO :: forall effs. (LastMember IO effs, Member Logger effs) => Handle -> ByteString -> Eff effs ()
hWriteIO handle content = do
          logM "FdWrite" [printf "writing %s to %s" (show content) (show handle)]
          sendM (BS.hPutStr handle content)

forkAndExecCmdIO :: forall effs. (LastMember IO effs, ViwrapEnv effs, Member Logger effs)
                 => HSlave -> Eff effs ProcessHandle
forkAndExecCmdIO sHandle = do
                 Env{envCmd} <- ask
                 logM "ForkAndExecCmd" [printf "starting process by executing %s cmd" envCmd]
                 (_,_,_,ph) <- sendM
                             $ Process.createProcess
                             $ (Process.shell envCmd) { delegate_ctlc = False
...
getTerminalAttrIO :: forall effs. (LastMember IO effs, Member Logger effs)
                  => Fd -> Eff effs TerminalAttributes
getTerminalAttrIO fd = do

                  termAttr <- sendM (Terminal.getTerminalAttributes fd)
                  logM "GetTerminalAttr" [printf "Terminal attributes of Fd: %d is: %s"
                                                 (toInteger fd)
...
setTerminalAttrIO :: forall effs. (LastMember IO effs, Member Logger effs)
                  => Fd -> TerminalAttributes -> TerminalState -> Eff effs ()
setTerminalAttrIO fd termAttr termState= do

                  logM "SetTerminalAttr" [printf "Setting terminal attributes: %s, for fd: %d"
                                                 (showTerminalModes termAttr)
                                                 (toInteger fd)
                                                 ]
...
allTerminalModes = [ ("InterruptOnBreak", InterruptOnBreak)
                   , ("MapCRtoLF", MapCRtoLF)
...
showTerminalModes termAttr = printf "Terminal Modes: %s" (show $ foldr acc [] allTerminalModes)
    ...
    where
        ...
        acc (name,mode) names
                | Terminal.terminalMode mode termAttr = name:names
                | otherwise                           = names
